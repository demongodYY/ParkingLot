...
_prototype.getParkinglotByRate = function() {
    var max_available_rate = 0;
    var parkinglot_index = -1;
    for (var i = 0; i < this.parkinglots.length; i++){
        var available_rate = this.parkinglots[i].available / this.parkinglots[i].capacity
        if(available_rate > max_available_rate){
            max_available_rate = available_rate;
            parkinglot_index = i;
        }
    }
